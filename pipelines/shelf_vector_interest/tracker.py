"""Generic person tracker (IoU + ReID) for the shelf-vector pipeline.

Deliberately self-contained -- a near-duplicate of the tracker in
`pipelines/shelf_interest/tracker.py`, kept local so this pipeline stays
free of imports from a sibling pipeline (see `pipelines/README.md`). It
only tracks position/appearance; it has no shelf-interest-specific state
(that lives in `scoring.py` / the pipeline's per-track state dict instead).
...
starting a fresh one, so a person moving between shelves keeps one ID.
"""
