"""Lightweight IoU tracking for the per-shelf interest pipeline."""
...
class Track:
    ...
    angle_hist: deque = field(default_factory=lambda: deque(maxlen=180))  # (frame, shelf_id, angle)

    # Shelf assignment / score
    shelf_id: str | None = None
    interest_ema: float = 0.0
    best_interest: float = 0.0

    # Candidate gating before opening a new event
    candidate_shelf_id: str | None = None
    candidate_run: int = 0

    # Current active event
    event_open: bool = False
    event_shelf_id: str | None = None
    event_start_frame: int | None = None
    event_last_strong_frame: int | None = None
    low_run: int = 0
    last_closed_shelf_id: str | None = None
    last_closed_frame: int | None = None

    # Counts and finished events
...
class IoUTracker:
    ...
    def update(
        ...
        if active and dets:
            # Stage 1: spatial continuity first. If IoU is good enough, keep the
            # same ID even when appearance changes (turning, bending, lighting).
            ...
            for c, i, j in iou_order:
                ...
                self._attach(
                    ...
                    frame_idx,
                    dets[i],
                    emb=(embeddings[i] if use_reid else None),
                    sim=iou_sim,
                )
                used_dets.add(i)
                assigned.add(tr.track_id)

            # Stage 2: ReID rescue for the leftovers (crossing, short occlusion).
            if use_reid:
        ...
        for i, det in enumerate(dets):
            if i in used_dets:
                continue
            # Re-link to a recently missed identity by appearance. This keeps
            # the same ID when IoU breaks due to crossing/occlusion.
            if use_reid:
                ...
                if relinked is not None:
                    ...
                    continue
                relinked_retired = self._try_reactivate_retired(frame_idx, det, embeddings[i], assigned)
                if relinked_retired is not None:
                    used_dets.add(i)
                    assigned.add(relinked_retired.track_id)
                    continue
            tid = self.next_id
            self.next_id += 1
            tr = Track(
    ...
    def _attach(self, tr: Track, frame_idx: int, det, emb: np.ndarray | None = None, sim: float = 1.0) -> None:
        prev_center = np.array([(tr.box[0] + tr.box[2]) * 0.5, (tr.box[1] + tr.box[3]) * 0.5], np.float32)
        new_center = np.array([(det.box[0] + det.box[2]) * 0.5, (det.box[1] + det.box[3]) * 0.5], np.float32)
    ...
    def _try_relink(
        self, frame_idx: int, det, emb: np.ndarray, assigned: set[int]
    ) -> Track | None:
        best = None
        best_score = -1.0
        ...
        for tr in self.tracks.values():
            ...
            if sim < self.relink_similarity:
                continue
            # Soft spatial sanity check to avoid identity jumps.
    ...
    def _try_reactivate_retired(
        ...
        if best is None:
            return None
        # Reactivate old identity with same ID.
